...
-- NatDex (PokeAPI) → species ID interno del juego (Gen III, pokeemerald decomp).
--
-- Gen I (1-151) y Gen II (152-251): internal == NatDex.
-- Gen III / Hoenn (252-386): orden interno = Hoenn Pokédex order, NO orden NatDex.
-- La tabla siguiente la generé directamente de
-- pret/pokeemerald/src/pokemon.c (sSpeciesToNationalPokedexNum), invertida.
local NATDEX_TO_INTERNAL_GEN3 = {
...
local function natdexToInternal(natdex)
  if natdex >= 252 and natdex <= 386 then
    return NATDEX_TO_INTERNAL_GEN3[natdex] or natdex
  end
  return natdex
end
